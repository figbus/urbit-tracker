module Codec exposing
    ...
    , CustomCodec, custom, variant0, variant1, variant2, buildCustom
    , oneOf
    , map
    , succeed, recursive, fail, andThen, lazy, value, build, constant
    , buildCustom0, custom0
    )
...
variant2 :
    ...
    -> CustomCodec c v
variant2 name ctor ( n1, m1 ) ( n2, m2 ) =
    variant name
        (\c v1 v2 ->
            c
                (JE.object
        ...
        (JD.map2 ctor
            (JD.field n1 <| decoder m1)
            (JD.field n2 <| decoder m2)
        )
    ...
    Codec
        ...
        , decoder =
            JD.dict JD.value
                |> JD.andThen
                    (\dict_ ->
                        case dict_ |> Dict.toList |> List.head of
                            Just ( tag, val ) ->
...
type Custom0Codec match v
    = Custom0Codec
        { match : match
        , decoder : Dict String v
        }


custom0 : match -> Custom0Codec match value
custom0 match =
    Custom0Codec
        { match = match
        , decoder = Dict.empty
...
variant0 :
    String
    -> v
    -> Custom0Codec (Value -> a) v
    -> Custom0Codec a v
variant0 name ctor (Custom0Codec am) =
    Custom0Codec
        { match = am.match <| JE.string name
        , decoder = Dict.insert name ctor am.decoder
        }


{-| Build a `Codec` for a fully specified custom type.
-}
buildCustom0 : Custom0Codec (a -> Value) a -> Codec a
buildCustom0 (Custom0Codec am) =
    Codec
        { encoder = \v -> am.match v
        , decoder =
            JD.string
